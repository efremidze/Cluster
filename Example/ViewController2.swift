...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let annotations: [Annotation] = (0..<1000).map { i in
            ...
            if i % 2 == 0 {
//                annotation.type = .image(named: "")
                annotation.type = .color(color: .blue, radius: 30)
            } else {
                annotation.type = .color(color: .red, radius: 20)
            }
            return annotation
        }
...
extension ViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if let annotation = annotation as? ClusterAnnotation {
            ...
            if view == nil {
                if let annotation = annotation.annotations.first as? CustomAnnotation {
                    view = ClusterAnnotationView(annotation: annotation, reuseIdentifier: identifier, type: annotation.type)
                } else {
